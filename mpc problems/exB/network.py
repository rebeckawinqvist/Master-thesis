...
class Network(nn.Module):
    ...
    def get_constraints(self, state, u_param):
        ...
        # constraints
        A = np.loadtxt('exB_A.csv', delimiter=',')
        B = np.loadtxt('exB_B.csv', delimiter=',')
        H = np.loadtxt('exB_cinfH.csv', delimiter=',')
        u_low = -5
        u_high = 5
        C_c = H[:,0:2]
        d_c = H[:,2]
...
if __name__ == "__main__":
    # data
    X = np.loadtxt('exB_input_data_grid.csv', delimiter=',')
    Y = np.loadtxt('exB_output_data_grid.csv', delimiter=',')
    print("Y: ", Y)
