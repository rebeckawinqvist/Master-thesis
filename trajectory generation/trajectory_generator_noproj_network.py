...
if __name__ == "__main__":
    ...
    for sample in initial_states:
        ...
        if n <= 2 and traj and plot_:
            ...
            plt.xlabel(xlabel)
            plt.ylabel(ylabel) 
            filen_fig = filename+"noprojNN_trajectory_{}_N_{}".format(s+1,N)+".png"

            plt.savefig(filen_fig)
            #plt.show()
        
        np.savetxt(filename+'noprojNN_trajectory_{}_N_{}'.format(s+1,N)+".csv", traj_matrix, delimiter=',')
        np.savetxt(filename+'noprojNN_controls_trajectory_{}_N_{}'.format(s+1,N)+".csv", u_matrix, delimiter=',')
        s += 1
