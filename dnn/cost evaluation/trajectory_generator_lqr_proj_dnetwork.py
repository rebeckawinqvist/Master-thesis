...
if __name__ == "__main__":
    ...
    for sample in initial_states:
        ...
        if n <= 2 and traj:
            ...
            plt.xlabel(xlabel)
            plt.ylabel(ylabel) 
            filen_fig = filename_save+"lqr_pdnn_ntrajs_{}_N_{}_traj_{}".format(ntrajs, N, s+1)+".png"

            plt.savefig(filen_fig)
            #plt.show()

        np.savetxt(filename_save+'lqr_pdnn_ntrajs_{}_N_{}_traj_{}'.format(ntrajs, N, s+1)+".csv", traj_matrix, delimiter=',')
        np.savetxt(filename_save+'lqr_pdnn_controls_ntrajs_{}_N_{}_traj_{}'.format(ntrajs, N, s+1)+".csv", u_matrix, delimiter=',')
        s += 1
