...
if __name__ == "__main__":
    ...
    NN = Network([8, 8, 8, 8])
    NN.load_state_dict(torch.load('ex{}/{}/networks/ex{}_pdnn_network_model.pt'.format(example, date, example)))
    ...
    for sample in initial_states:
        ...
        if n <= 2 and traj:
            ...
            plt.xlabel(xlabel)
            plt.ylabel(ylabel) 
            filen_fig = filename_save+"pdnn_ntrajs_{}_nsim_{}_traj_{}".format(ntrajs, nsim, s+1)+".png"

            plt.savefig(filen_fig)
            #plt.show()

        np.savetxt(filename_save+'pdnn_ntrajs_{}_nsim_{}_traj_{}'.format(ntrajs, nsim, s+1)+".csv", traj_matrix, delimiter=',')
        np.savetxt(filename_save+'pdnn_controls_ntrajs_{}_nsim_{}_traj_{}'.format(ntrajs, nsim, s+1)+".csv", u_matrix, delimiter=',')
        s += 1
