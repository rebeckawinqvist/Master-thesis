...
filename_save = "ex{}/d{}/trajectories/".format(example, date)
...
if __name__ == "__main__":
    ...
    NN = Network([8,8])
    NN.load_state_dict(torch.load('ex{}/d{}/networks/ex{}_proj_network_model_new.pt'.format(example, date, example)))
    ...
    initial_states = np.loadtxt("ex{}/initial_states/ex{}_init_states_trajectories_ntrajs_{}_N_{}.csv".format(example, example, ntrajs, N), delimiter=',')
    s = 0
    for sample in initial_states:
        ...
        if n <= 2 and traj:
            ...
            title = "Example {}: \n Sample: {} \n Proj NN trajectory".format(example,s+1)
            xlabel = "$x_1$"
            ylabel = "$x_2$"
            plt.title(title)
            plt.xlabel(xlabel)
            plt.ylabel(ylabel) 
            filen_fig = filename_save+"projNN_ntrajs_{}_N_{}_traj_{}".format(ntrajs, N, s+1)+".png"

            plt.savefig(filen_fig)
            #plt.show()

        np.savetxt(filename_save+'projNN_ntrajs_{}_N_{}_traj_{}'.format(ntrajs, N, s+1)+".csv", traj_matrix, delimiter=',')
        np.savetxt(filename_save+'projNN_controls_ntrajs_{}_N_{}_traj_{}'.format(ntrajs, N, s+1)+".csv", u_matrix, delimiter=',')
        s += 1
