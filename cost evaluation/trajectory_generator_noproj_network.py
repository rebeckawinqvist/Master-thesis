...
if __name__ == "__main__":
    ...
    NN = Network([8,8])
    NN.load_state_dict(torch.load('ex{}/d{}/networks/ex{}_noproj_network_model_new.pt'.format(example, date, example)))
    ...
    for sample in initial_states:
        ...
        if n <= 2:
            ...
            plt.xlabel(xlabel)
            plt.ylabel(ylabel) 
            filen_fig = filename_save+"noprojNN_ntrajs_{}_N_{}_traj_{}".format(ntrajs, N, s+1)+".png"

            plt.savefig(filen_fig)
            #plt.show()
        
        np.savetxt(filename_save+'noprojNN_ntrajs_{}_N_{}_traj_{}'.format(ntrajs, N, s+1)+".csv", traj_matrix, delimiter=',')
        np.savetxt(filename_save+'noprojNN_controls_ntrajs_{}_N_{}_traj_{}'.format(ntrajs, N, s+1)+".csv", u_matrix, delimiter=',')
        s += 1
